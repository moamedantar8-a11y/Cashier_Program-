...
<body>
    ...
    <div class="phone-frame">
        ...
        <div id="screen-welcome" class="screen welcome-screen active">
            ...
            <p style="font-size: 11px; color: var(--text-muted); margin-bottom: 5px;">المنصة التعليمية الذكية - أولى ثانوي[cite: 1]</p>
            
            <!-- ميزة بحث سريعة بين الحسابات المحفوظة -->
            <div style="width: 100%; margin-bottom: 6px;">
                <input type="text" id="searchSavedInput" placeholder="🔍 بحث عن اسم طالب مسجل..." oninput="filterSavedAccounts()" style="width:100%; padding:6px 10px; font-size:10px; border-radius:8px; border:1px solid #cbd5e1; outline:none;">
            </div>

            <div class="accounts-list" id="savedAccountsContainer"></div>

            <div style="width: 100%; margin-top: auto; padding-bottom: 5px;">
        ...
        <div id="screen-register" class="screen register-screen">
            ...
            <p style="font-size: 9px; color: #cbd5e1; text-align: center; margin-bottom: 6px;">اختر المدرس والمجموعة بدقة</p>
            ...
            <div class="dashboard-content">
                ...
                <div id="section-profile">
                    ...
                    <div id="teacherDeleteAccountBox" style="margin-top: 10px; display: none;">
                        <button class="btn-main" style="background: var(--accent-red); font-size: 10px; padding: 6px;" onclick="deleteCurrentStudentAccount()"><i class="fa-solid fa-trash"></i> حذف هذا الطالب من قاعدة بيانات المعلم</button>
                    </div>
                </div>
                ...
                <div id="section-community" style="display: none;">
                    <div style="display: flex; flex-direction: column; gap: 6px;">
                        ...
                        <p style="font-size: 9px; color: var(--text-muted);">مجتمع خاص بطلاب هذه المادة فقط.</p>
    ...
    <div class="modal-overlay" id="teacherLoginModal" onclick="closeTeacherModal(event)">
        <div class="modal-card" onclick="event.stopPropagation()" style="text-align: right;">
            ...
            <p style="font-size: 9px; color: var(--text-muted); margin-bottom: 10px;">كل معلم يدخل بكود سري ولا يرى سوى طلاب مادته.</p>
    ...
    <script>
        ...
        function renderSavedAccounts(filterText = "") {
            const container = document.getElementById('savedAccountsContainer');
            container.innerHTML = '';
            
            let filtered = savedAccounts.filter(acc => acc.name.includes(filterText) || acc.teacher.includes(filterText));

            if(filtered.length === 0) {
                container.innerHTML = '<p style="font-size: 10px; color: var(--text-muted); text-align: center; margin-top: 15px;">لا توجد حسابات مطابقة للبحث.<br>قم بتسجيل طالب جديد بالسنتر.</p>';
                return;
            }

            filtered.forEach((acc, index) => {
                let realIndex = savedAccounts.indexOf(acc);
                container.innerHTML += `
                    <div class="account-card-saved" onclick="loginStudent(${realIndex})">
                        ...
                        <div style="display: flex; align-items: center; gap: 6px;">
                            <button onclick="event.stopPropagation(); deleteAccount(${realIndex})" style="background: #fee2e2; color: var(--accent-red); border: none; width: 26px; height: 26px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center;" title="حذف"><i class="fa-solid fa-trash" style="font-size: 10px;"></i></button>
        ...
        function filterSavedAccounts() {
            let query = document.getElementById('searchSavedInput').value.trim();
            renderSavedAccounts(query);
        }
        ...
        function renderCommunityChat() {
            ...
            let currentMessages = communityMessages[roomKey] || [
                { sender: "إدارة المادة", text: `أهلاً بك في مجتمع مادة ${acc.teacher} لأولى ثانوي.`, time: "اليوم" }
            ];
